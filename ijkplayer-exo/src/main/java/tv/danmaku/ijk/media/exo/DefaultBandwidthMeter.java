...
public class DefaultBandwidthMeter implements BandwidthMeter, TransferListener {
  ...
  public static final class Builder {
    ...
    public Builder(Context context) {
      ...
      percentile = DEFAULT_SLIDING_WINDOW_PERCENTILE;
      clock = Clock.DEFAULT;
    ...
    public Builder setSlideWindowPercentile(float percentile) {
  ...
  private DefaultBandwidthMeter(
    ...
    Log.d("DefaultBandwidthMeter", "maxWeight=" + maxWeight + ", percentile=" + percentile);
  }
  ...
  public synchronized void onTransferEnd(DataSource source, DataSpec dataSpec, boolean isNetwork) {
    ...
    if (sampleElapsedTimeMs > 0) {
      ...
      if (totalElapsedTimeMs >= ELAPSED_MILLIS_FOR_ESTIMATE
        ...
        bitrateEstimate = (long) slidingPercentile.getPercentile(percentile);
        Log.d("DefaultBandwidthMeter", "onTransferEnd bitrateEstimate changed to " + bitrateEstimate);
      }
  ...
  public synchronized void addSample(int dataSize, int sampleElapsedTimeMs) {
    totalElapsedTimeMs += sampleElapsedTimeMs;
    totalBytesTransferred += dataSize;
    if (sampleElapsedTimeMs > 0) {
      float bitsPerSecond = (dataSize * 8000f) / sampleElapsedTimeMs;
      slidingPercentile.addSample((int) Math.sqrt(dataSize), bitsPerSecond);
      if (totalElapsedTimeMs >= ELAPSED_MILLIS_FOR_ESTIMATE
              || totalBytesTransferred >= BYTES_TRANSFERRED_FOR_ESTIMATE) {
        bitrateEstimate = (long) slidingPercentile.getPercentile(percentile);
        Log.d("DefaultBandwidthMeter", "bitrateEstimate changed to " + bitrateEstimate);
      }
    }
  }
